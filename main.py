...
Spuštění:
    pip install fastapi uvicorn aiosmtplib jinja2 python-multipart httpx
...
ADMIN_PASSWORD = hashlib.sha256(os.environ.get("ADMIN_PASSWORD", "").encode()).hexdigest()
SECRET_TOKEN   = os.environ.get("SECRET_TOKEN", "stenico-secret-2025")

DB_PATH = os.environ.get("DB_PATH", "/app/data/stenico.db")

# Zajistí, že adresář pro databázi existuje (i bez připojeného Volume)
...
def email_host_html(r: dict) -> str:
    ...
    <div style="font-family:sans-serif;max-width:600px;margin:auto">
      ...
      <p style="margin-top:1rem">
        <a href="https://web-production-5a5e.up.railway.app/admin" style="background:#c9a84c;color:#fff;padding:10px 20px;text-decoration:none">
          Otevřít admin panel
        </a>
      </p>
    </div>
...
async def odeslat_poptavku(data: PoptavkaIn, db: sqlite3.Connection = Depends(get_db)):
    ...
    # Kontrola dostupnosti – jen proti POTVRZENÝM rezervacím.
    # (Nepotvrzené poptávky termín neblokují – víc lidí se může ptát na stejný termín.)
    existing = db.execute("""
...
async def dostupnost(db: sqlite3.Connection = Depends(get_db)):
    # Blokujeme jen POTVRZENÉ rezervace – ty se propisují z admin panelu.
    rows = db.execute("""
        SELECT datum_od, datum_do FROM rezervace WHERE stav = 'potvrzena'
    """).fetchall()
    ...
    for r in rows:
        ...
        # Den odjezdu zůstává volný (nový host může týž den přijet)
        while d < end:
            obsazeno.add(str(d))
            d += timedelta(days=1)
    for b in bloky:
